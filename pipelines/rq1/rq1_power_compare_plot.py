...
Reads:
  results/rq1/combo_power_equal_weight/test_actions.csv            (p2)
  results/rq1_power16/combo_power16_equal_weight/test_actions.csv  (p16)
  results/rq1_power32/combo_power32_equal_weight/test_actions.csv  (p32)
and same for previous safe.

Produces (for each safe):
  results/rq1_power16/confidence_compare_p2_p16_p32_{safe}.png
...
def load(safe, p):
    if p == 2:
        path = f"results/rq1/combo_power_equal_weight/test_actions.csv" if safe=="equal_weight" else f"results/rq1/combo_power_previous/test_actions.csv"
        # Note canonical previous exists under rq1/combo_power_previous
        if safe == "previous":
            # fallback: check both
            alt = "results/rq1/combo_power_previous/test_actions.csv"
            if not os.path.exists(path):
                path = alt
    else:
        path = f"results/rq1_power{p}/combo_power{p}_{safe}/test_actions.csv"
    df = pd.read_csv(path)
...
def plot_for_safe(safe):
    ...
    out1 = f"results/rq1_power16/confidence_compare_p2_p16_p32_{safe}.png"
